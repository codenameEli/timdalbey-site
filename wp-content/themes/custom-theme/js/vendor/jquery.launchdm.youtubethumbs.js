 ...
 *
 * 1.0.9 modify treatment of youtube link with an image inside it - new behavior recreates the same markup
 *          as generated when a thumbnail from youtube is used
    ...
    $.fn.launchdmYouTubeThumbs = function(options) {
        ...
        return this.each(function(key, value) {
                ...
                )
            {
                ...
                // do NOT add thumbnail or play button if there is the class .youtube_item_suppress_thumbnail
                if ( $this.hasClass('youtube_item_suppress_thumbnail') ) {
                    if ( video_name ) {
                ...
                // apply playbutton and styling
                $image = $this.find('img');
                if (
                    // has internal images
                    $image.length
                ) {
                    $image.addClass('thumbnail').wrap('<span class="image_frame" />');

                    $this.addClass('youtube_item');

                    // add play button
                    $this.prepend("<i class='fa fa-play green-overlay'><!-- playbutton --></i>");
                ...
                // add play button
                $this.prepend("<i class='fa fa-play green-overlay'></i>");
